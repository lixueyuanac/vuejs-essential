...
import { article,getUserByUserName,getUserByID,articles } from '@/api/api'
...
export default {
  ...
  methods: {
    async setDataByParams(params) {
      ...
      if (articleId) {
        let articleApi = await article(articleId)
        const userid = articleApi['data']['uid']
        const user = await getUserByID({'id':userid})
        this.userName = user.data.username
        this.userAvatar = user.data.avatar
        const apiArticles = await articles({'uid':user.data.id})
        this.articles = apiArticles['data']
