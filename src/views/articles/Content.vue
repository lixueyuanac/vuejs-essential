...
export default {
  ...
  computed: {
   ...
   created() {
    ...
    //const article = this.$store.getters.getArticleById(articleId)
        article(articleId).then(res=> {
        console.log(res.data.content)
        this.article = res
        this.title = res.data.title
        this.content = SimpleMDE.prototype.markdown(emoji.emojify(res.data.content, name => name))
    ...
    //   // 更新 likeClass, 点赞用户列表包含当前用户是，赋值为 active, 表示已赞
  ...
  beforeRouteEnter(to, from, next) { // 在路由进入之前获取是否是添加或修改
    const fromName = from.name
    console.log(to,from)
    next(vm => {
      switch (fromName) {
          case 'Create':
            vm.$message.show('添加成功')

            break
          case 'Edit':
            vm.$message.show('修改成功')
            break
      }
    })
  },
  ...
  methods: {
    editArticle() {
      ...
      this.$router.push({name:'Edit', path: `/articles/${this.articleId}/edit`})
